...
def plot_roc_curve(y_true, y_score, model_dir):
    ...
    plt.savefig(model_dir + '/pfnpre_roc_test.png')
    return fpr, tpr, roc_auc

def save_roc_data(fpr, tpr, roc_auc, model_dir):
    np.savez(os.path.join(model_dir, 'pfnpre_roc_data_test.npz'),
             fpr=fpr, tpr=tpr, roc_auc=roc_auc)

def save_auc_score(model_dir, auc_score):
    with open(model_dir + '/pfnpre_auc.txt', 'w') as file:
        file.write(f'pfnpre_auc_score\n{auc_score}\n')
