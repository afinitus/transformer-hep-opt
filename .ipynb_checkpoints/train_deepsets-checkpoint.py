...
from model_deepsets import DeepSetsAttClass  # Ensure this is imported correctly
...
def plot_rocs(model, val_loader, tag):
    ...
    fpr = np.clip(fpr, 1e-6, 1)  # Avoid division by zero
...
def save_model(model, log_dir, name):
    torch.save(model.state_dict(), os.path.join(log_dir, f"model_{name}.pt"))

def load_and_evaluate_model(model_class, log_dir, name, device, args):
    model = model_class(
        num_feat=3,
    ...
    model.load_state_dict(torch.load(os.path.join(log_dir, f"model_{name}.pt"), map_location=device))
    model.to(device)
    return model
...
if __name__ == "__main__":
    ...
    for epoch in range(args.num_epochs):
        ...
        for x, padding_mask, label in tqdm(
            ...
            with torch.cuda.amp.autocast():
                logits = model(x)
                loss = nn.BCEWithLogitsLoss()(logits.squeeze(), label)
    ...
    model = load_and_evaluate_model(
        DeepSetsAttClass, args.log_dir, "best", device, args
    )
